...
def run_quality_v3(project_root:Path)->QualityV3Report:
    findings=[]; files=list((project_root/'artifacts').rglob('*.md'))+list((project_root/'exports/customer').rglob('*.md'))
    for p in files:
...
def write_quality_v3(project_root:Path)->Path:
    r=run_quality_v3(project_root); out=project_root/'quality/customer-review-v3.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(asdict(r),ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    ...
    (project_root/'quality/customer-review-v3.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
    return out
