...
def export_docx(project_root: Path, profile: str = "client-ready", force: bool = False) -> Path:
    ...
    _add_cover_page(doc, project_root, meta, profile)
    _add_governance_tables(doc, project_root, meta, profile)
...
def _configure_doc_styles(doc) -> None:
    section = doc.sections[0]
    section.top_margin = section.bottom_margin = section.left_margin = section.right_margin = 914400  # 1 inch
    ...
    header.text = "PMO Studio Documentation Pack"
    footer = section.footer.paragraphs[0]
    footer.text = "Generated by PMO Studio | Confidential"


def _add_cover_page(doc, project_root: Path, meta: dict, profile: str) -> None:
    ...
    doc.add_paragraph(f"Product: {', '.join(meta.get('products', []) or []) or 'N/A'}")
    doc.add_paragraph(f"Profile: {profile}")
    doc.add_paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    doc.add_paragraph("Status: Customer-ready generated package. Review/sign-off state is included below when available.")
    doc.add_page_break()


def _add_governance_tables(doc, project_root: Path, meta: dict, profile: str) -> None:
    doc.add_heading("Document Control", level=1)
    rows = [
        ("Project", meta.get('project_slug', project_root.name)),
    ...
    if manifest:
        rows.extend([
            ...
            ("Domain pack", manifest.get('domain_pack', '')),
            ("Source hash", manifest.get('source_hash', '')[:16]),
            ("Template hash", manifest.get('template_tree_hash', '')[:16]),
        ])
    table = doc.add_table(rows=len(rows), cols=2); table.style = "Table Grid"
    for i, (k, v) in enumerate(rows):
        table.cell(i, 0).text = str(k); table.cell(i, 1).text = str(v)
    doc.add_heading("Approval / Sign-off", level=1)
    signoff = _json(project_root / 'review/signoff.json') or {}
    approvals = signoff.get('approvals') or {}
    if approvals:
        table = doc.add_table(rows=len(approvals)+1, cols=5); table.style = "Table Grid"
        for c, h in enumerate(["Role", "Status", "By", "Updated", "Note"]): table.cell(0,c).text = h
        for r, (role, entry) in enumerate(sorted(approvals.items()), 1):
            table.cell(r,0).text = role; table.cell(r,1).text = str(entry.get('status',''))
            table.cell(r,2).text = str(entry.get('approved_by','')); table.cell(r,3).text = str(entry.get('updated_at',''))
            table.cell(r,4).text = str(entry.get('note',''))
    else:
        doc.add_paragraph("No sign-off state recorded.")
    doc.add_heading("Change History", level=1)
    table = doc.add_table(rows=2, cols=4); table.style = "Table Grid"
    for c, h in enumerate(["Version", "Date", "Author", "Change"]): table.cell(0,c).text = h
    table.cell(1,0).text = str(manifest.get('generator_version', 'draft')); table.cell(1,1).text = datetime.now().strftime('%Y-%m-%d')
    table.cell(1,2).text = "PMO Studio"; table.cell(1,3).text = "Generated documentation pack"
    doc.add_page_break()
