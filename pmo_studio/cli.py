...
def cmd_detailed_quote(args):
    args.slug = _resolve_slug(args); p = Project.load(args.slug, root_base=Path(args.root)); print(f"Detailed quotation: {generate_detailed_quotation(p, args.lang)}")
...
def cmd_decomposition_review(args):
    args.slug = _resolve_slug(args)
    p = Project.load(args.slug, root_base=Path(args.root))
    out = write_decomposition_review(p.root)
    print(f"Decomposition review: {out}")
...
def cmd_decompose(args):
    args.slug = _resolve_slug(args)
    p = Project.load(args.slug, root_base=Path(args.root))
    out = write_decomposition(p.root)
    print(f"Functional decomposition: {out}")
...
def build_parser():
    ...
    dq = sub.add_parser("detailed-quote", help="Run v5.1 Customer Detailed Quotation from BA Model"); dq.add_argument("slug", nargs="?"); dq.add_argument("--lang", default="vi"); dq.set_defaults(func=cmd_detailed_quote)
    ...
    dr = sub.add_parser("decomp-review", help="Run v3.2 decomposition review mode")
    dr.add_argument("slug", nargs="?"); dr.set_defaults(func=cmd_decomposition_review)
